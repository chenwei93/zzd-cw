Ext.define('DA.view.dataservice.entry.EntryEditController', {
    ...
    onSaveGriddataset: function () {
        ...
        for (var i = 0; i < store.length; i++) {
            ...
            if (store[i].attributes == 'xx') {
                store[i].attributes = {
                    editor: 'attachment_name',
                    renderer: 'attachment_name'
                }
            }

            if (store[i].alias == true) {
                store[i].alias = 'id';
            }
        }
        if (name == "") {
    ...
    onNewGriddataset: function () {
        ...
        var addNewData = function () {
            var rec = new DA.model.GridDataset({
                name: '',
                title: '',
                dataLength: '',
                notNull: '',
    ...
    onCHooseYwType: function (view, tr, td, index, a, record) {
        var data = record.get('attributes');
        if (data && data.editor) {
            var jude = data.editor.substring(0, data.editor.indexOf(':'));
        }
        if (record.get('attributes') == 'fj' || jude == 'attachment') {
            this.open({
    ...
    onGridDatasetsRender: function (grid) {
        var entityId = grid.up('entry-edit').entityId;
        if (entityId) {
            Ext.Ajax.request({
                ...
                success: function (rs) {
                    ...
                    if (rs.hasOwnProperty('fields')) {
                        if (rs.fields.hasOwnProperty('items')) {
                            top._fieldItem = rs.fields.items;
